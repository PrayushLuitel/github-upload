...
### run chart of measurements
df_01 %>% 
  ggplot(mapping = aes(x = obs_id, y = x)) + 
  geom_line() + 
  geom_point(mapping = aes(x = obs_id))
```
...
df_01 %>% 
  ...
  geom_histogram(bins= 25, color = "white", fill = "black", size = .75)
```
Based on the run chart and the histogram, most of the values are concentrated around the middle or the value 0. This suggests that the shape of the distribution is close to a Gaussian.
...
quantile(df_01 $x, c(0.05, 0.95))

### calculate the empirical standard deviation of x
sd(df_01 $x)
```

Based on the summary statistics, the value of x is centered around 0 as given by the mean and median. Furthermore, the 5% and 95% quantiles suggests that 90% of the values are within -7.79 and 9.23. This is in line with my conclusion based on the run chart and histogram as the distribution is representative of the Gaussian.
...
df_01 %>%
  mutate(rolling_avg = cummean(x)) %>%
  ggplot(mapping = aes(x = obs_id, y= rolling_avg)) + geom_line()
```
The first fifty observations in the rolling average seem to be the most variable but after this and definately by 200 observations the rolling average is more constant, with small variations.
...
mydf <- subset(df_01, obs_id <6)
mean(mydf$x)
mydf <- subset(df_01, obs_id <46)
mean(mydf$x)
```
...
#### SOLUTION
The Laplace approximation approximates the posterior distribution as a Gaussian/normal. However, this is not appropriate because sigma has a natural lower bound of 0 but a Gaussian allows any positive or negative value. Thus, the Laplace approximation does not keep the lower bound, and therefore we need to use a transformation from sigma to varphi to respect the lower bound of 0, so that if the value of sigma needed to be negative, it could.
...
$$
p\left(\mu, \varphi \mid  \mathbf{x} \right) \propto \prod_{n=1}^{N} \left( \mathrm{normal}\left( x_n \mid \mu,g^{-1} \left( \varphi \right) \right) \right) \times p\left(\mu\right) \times p\left( g^{-1} \left( \varphi \right) \right) \times \left( \left| \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right| \right)
$$
...
viz_logpost_surface(log_post_result = log_post_inform_N05, grid_values = param_grid)
```

Any value less than approximately 1 for the transformed unbounded varphi parameter are ruled out in the case with five observations. This is because any value of mu with a varphi value less than 1 is greyed out in the graph above. Also, values between -.5 and 0.5 appear to be the most probable for the unknown mean mu, as the yellow filled section of the graph has a width around this range. With just 5 observations, the posterior uncertainty is much larger than the prior uncertainty.
...
viz_logpost_surface(log_post_result = log_post_inform_N45, grid_values = param_grid)
```

```{r, solution_02g_c, eval=TRUE}
viz_logpost_surface(log_post_result = log_post_inform_N405, grid_values = param_grid)
```

The log posterior surface becomes much more restricted as the sample size increases. With just 5 observations, there was a large combination of values for mu and varphi that could be plausible. However, as the sample size increased to 45 and eventually 405, the combination of values for mu and varphi that could be plausible significantly declined. As the sample size increased the most probable values for varphi changed from being greater than 1.5 to closer to 1.5 to being very much 1.75. Similarly, the values for mu went from a range between -0.5 to 0.5 with the most probable value being 0 to being close to 0.5 to finally being 0.6. Thus, as the sample size increases, the uncertainty in the parameters decrease.
...
The identified optimal parameters are the same for both cases, as with the hessian matricies. The only apparent difference was in the counts section where the values of the function and gradient differ slightly. Since the log-posterior surface had a clear maximum for the parameters, it was clear that no matter what the initial guess was, the final result would provide the same value for the optimized parameters.
...
laplace_res_inform_N05 <- my_laplace(init_guess_02, my_logpost, info_inform_N05)

laplace_res_inform_N45 <- my_laplace(init_guess_02, my_logpost, info_inform_N45)

laplace_res_inform_N405 <- my_laplace(init_guess_02, my_logpost, info_inform_N405)

### check if they eached converged
laplace_res_inform_N05
laplace_res_inform_N45
laplace_res_inform_N405
```
All three converged.
...
```{r, solution_03fa_1, eval=TRUE}
summary(post_samples_inform_N05)
```
```{r, solution_03fa_2, eval=TRUE}
post_samples_inform_N05 %>%
  ggplot(mapping = aes(x= mu)) +
  geom_histogram(bins = 55) +
  theme_bw()
```
```{r, solution_03fa_3, eval=TRUE}
post_samples_inform_N05 %>%
...
```{r, solution_03fb_1, eval=TRUE}
summary(post_samples_inform_N45)
```
```{r, solution_03fb_2, eval=TRUE}
...
```{r, solution_03fc_1, eval=TRUE}
summary(post_samples_inform_N405)
```
```{r, solution_03fc_2, eval=TRUE}
post_samples_inform_N405 %>%
  ggplot(mapping = aes(x= mu)) +
  geom_histogram(bins = 55) +
  theme_bw()
```
```{r, solution_03fc_3, eval=TRUE}
post_samples_inform_N405 %>%
  ggplot(mapping = aes(x= sigma)) +
  geom_histogram(bins = 55) +
  theme_bw()
...
#### SOLUTION
```{r, solution_03g_1_a, eval=TRUE}
temporary1 <-filter(post_samples_inform_N05, mu >=0)
count(temporary1)/count(post_samples_inform_N05)
```
```{r, solution_03g_1_b, eval=TRUE}
quantile(post_samples_inform_N05$sigma, c(0.05, 0.95))
```
```{r, solution_03g_2_a, eval =TRUE}
temporary2 <-filter(post_samples_inform_N45, mu >=0)
count(temporary2)/count(post_samples_inform_N45)
```

```{r, solution_03g_2_b, eval=TRUE}

quantile(post_samples_inform_N45$sigma, c(0.05, 0.95))

```
```{r, solution_03g_3_a, eval=TRUE}
temporary3 <-filter(post_samples_inform_N405, mu >=0)
count(temporary3)/count(post_samples_inform_N405)

```

```{r, solution_03g_3_b, eval=TRUE}
...
Your solution here.  
```{r, solution_4a1, eval=TRUE}
df_02_low %>% 
  ggplot(mapping = aes(x=x, y=y)) +
  geom_point()
```
```{r, solution_4a2, eval=TRUE}
df_02_high %>% 
  ggplot(mapping = aes(x=x, y=y)) +
  geom_point()
```
The funcitonal form that has low noise is a polynomial function. Therefore I believe that the true funcitonal form of the low noise data set is a polynomial.
...
mod_low_linear <- lm(y ~ x, df_02_low)

mod_low_quad <- lm(y ~ x + I(x^2), df_02_low)

mod_low_8th <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), df_02_low)
```

The high noise case models are given in the code chunk below.  

```{r, solution_04b_b, eval=TRUE}
mod_high_linear <- lm(y ~ x, df_02_high)

mod_high_quad <- lm(y ~ x + I(x^2), df_02_high)

mod_high_8th <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), df_02_high)
```
...
```
```{r, solution_04c_e, eval=TRUE}
summary(mod_high_quad)
```
```{r, solution_04c_f, eval=TRUE}
summary(mod_high_8th)
```
For the low noise case the quadratic and the 8th order polynomial funciton had similarly high r squared values, however for the high noise case the 8th order polynomial function had the highest r squared values.
...
modelr::rmse(mod_low_linear, df_02_low)
modelr::rmse(mod_low_quad, df_02_low )
modelr::rmse(mod_low_8th, df_02_low )
```

The high noise case RMSEs are calculated below.  

```{r, solution_04d_b, eval=TRUE}
modelr::rmse(mod_high_linear, df_02_high)
modelr::rmse(mod_high_quad, df_02_high)
modelr::rmse(mod_high_8th, df_02_high)
```
...
mod_low_quad_fold_01 <- lm(y ~ x + I(x^2), low_noise_train_fold_01)

mod_low_quad_fold_02 <-lm(y ~ x + I(x^2), low_noise_train_fold_02) 

mod_low_quad_fold_03 <- lm(y ~ x + I(x^2), low_noise_train_fold_03)

mod_low_quad_fold_04 <- lm(y ~ x + I(x^2), low_noise_train_fold_04)

mod_low_quad_fold_05 <- lm(y ~ x + I(x^2), low_noise_train_fold_05)
```

The 8th order polynomial fits should be specified below.  

```{r, solution_04f_b, eval=TRUE}
mod_low_8th_fold_01 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), low_noise_train_fold_01)

mod_low_8th_fold_02 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), low_noise_train_fold_02)

mod_low_8th_fold_03 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), low_noise_train_fold_03)

mod_low_8th_fold_04 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), low_noise_train_fold_04)

mod_low_8th_fold_05 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), low_noise_train_fold_05)
```
...
cv_low_quad_fold_train_rmse <- c(modelr::rmse(mod_low_quad_fold_01, df_02_low),
modelr::rmse(mod_low_quad_fold_02, df_02_low),
modelr::rmse(mod_low_quad_fold_03, df_02_low),
modelr::rmse(mod_low_quad_fold_04, df_02_low),
modelr::rmse(mod_low_quad_fold_05, df_02_low))
```
```{r, solution_04g_b, eval=TRUE}
cv_low_8th_fold_train_rmse <- c(modelr::rmse(mod_low_8th_fold_01, df_02_low),
modelr::rmse(mod_low_8th_fold_02, df_02_low),
modelr::rmse(mod_low_8th_fold_03, df_02_low),
modelr::rmse(mod_low_8th_fold_04, df_02_low),
modelr::rmse(mod_low_8th_fold_05, df_02_low))
```
```{r, solution_04g_c, eval=TRUE}
mean(cv_low_quad_fold_train_rmse)
```
```{r, solution_04g_d, eval=TRUE}
mean(cv_low_8th_fold_train_rmse)
```
The quadratic relationship has the lowest average RMSE between the quadratic relationship and 8th order polynomial relationship for the low noise training datasets.
...
mod_low_quad_fold_001 <- lm(y ~ x + I(x^2), low_noise_test_fold_01)

mod_low_quad_fold_002 <-lm(y ~ x + I(x^2), low_noise_test_fold_02) 

mod_low_quad_fold_003 <- lm(y ~ x + I(x^2), low_noise_test_fold_03)

mod_low_quad_fold_004 <- lm(y ~ x + I(x^2), low_noise_test_fold_04)

mod_low_quad_fold_005 <- lm(y ~ x + I(x^2), low_noise_test_fold_05)
```

The 8th order polynomial fits should be specified below.  

```{r, solution_04h_b, eval=TRUE}
mod_low_8th_fold_001 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), low_noise_test_fold_01)

mod_low_8th_fold_002 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), low_noise_test_fold_02)

mod_low_8th_fold_003 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), low_noise_test_fold_03)

mod_low_8th_fold_004 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), low_noise_test_fold_04)

mod_low_8th_fold_005 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), low_noise_test_fold_05)
```
```{r, solution_04h_c, eval=TRUE}
cv_low_quad_fold_test_rmse <- c(modelr::rmse(mod_low_quad_fold_001, df_02_low),
modelr::rmse(mod_low_quad_fold_002, df_02_low),
modelr::rmse(mod_low_quad_fold_003, df_02_low),
modelr::rmse(mod_low_quad_fold_004, df_02_low),
modelr::rmse(mod_low_quad_fold_005, df_02_low))
```
```{r, solution_04h_d, eval=TRUE}
cv_low_8th_fold_test_rmse <- c(modelr::rmse(mod_low_8th_fold_001, df_02_low),
modelr::rmse(mod_low_8th_fold_002, df_02_low),
modelr::rmse(mod_low_8th_fold_003, df_02_low),
modelr::rmse(mod_low_8th_fold_004, df_02_low),
modelr::rmse(mod_low_8th_fold_005, df_02_low))
```
```{r, solution_04h_e, eval=TRUE}
mean(cv_low_quad_fold_test_rmse)
```
```{r, solution_04h_f, eval=TRUE}
